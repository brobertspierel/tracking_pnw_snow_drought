...
# 	output_dict = {}
	...
	for x in range(3):
		...
		for y in range(3):  
			...
			# corr, _ = pearsonr(df['d_mod'], df['s_mod'])
	...
	# plt.show()
	# plt.close('all')
	fig_fn = os.path.join(kwargs.get('fig_dir'),f'{kwargs.get("id_col")}_daymet_snotel_{kwargs.get("pred_var")}_comparison_draft2.jpg')
...
if __name__ == '__main__':
	...
	main(daymet_dir,pickles,id_col='site_num',
							hucs=hucs_dict,
							stats_dir=stats_dir,
							fig_dir=fig_dir,
							pred_var='swe'
