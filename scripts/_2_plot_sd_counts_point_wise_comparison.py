...
def plot_counts(df_list,output_dir,huc_col='huc8',**kwargs): 
	...
	for x in range(3): 
		for y in range(3): 
			...
			d_counts = df_list[x][f'ua_{cols[y]}'].value_counts().sort_index().astype('int')
			# print(s_counts)
			# print(d_counts)
			df = pd.DataFrame({"snotel":s_counts,"ua_swe":d_counts})
			...
			mk_dict.update({f's_{ylabels[x]}_{xlabels[y]}':mk_test(df.snotel)[0],
				f'd_{ylabels[x]}_{xlabels[y]}':mk_test(df.ua_swe)[0]})
