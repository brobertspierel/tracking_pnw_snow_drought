...
class FormatData(): 
	def __init__(self,input_files,time_period=None,drop_cols=None,sel_cols=None,date_col='date'): 
	...
	def read_in_csvs(self): 
		...
		if self.drop_cols: 
			...
			try: 
				output = output[self.sel_cols]
				return output
			except IndexError: 
				return output
		else: 
			print('not dropping or selecting cols')
			return output
...
class CalcSnowDroughts(): 
	...
	"""
	def __init__(self,input_df,swe_c='swe',precip='ppt',temp='tmean',date_col='date',sort_col='huc8',start_year=1990): 
